...
public class LMouseClicked extends MouseAdapter
{
	boolean isWhite;
	...
	LMouseClicked(boolean z, ColorLabel arrayLabelW[][], ColorLabel arrayLabelB[][], int a, int b, int x, JButton buttonW, JButton buttonB, JLabel subW, JLabel subB)
	{
		isWhite = z;
	...
	{	
		if(isWhite == true)
		{
			for(int i = x; i >= 1; i--)
			{
				arrayLabelW[a][b - i].setWhite();
				arrayLabelB[7 - a][7 - (b - i)].setWhite();
			}
		...
		else
		{
			for(int i = x; i >= 1; i--)
			{
				arrayLabelB[a][b - i].setBlack();
				arrayLabelW[7 - a][7 - (b - i)].setBlack();
			}
