...
public class TMouseClicked extends MouseAdapter
{
	boolean isWhite;
	...
	TMouseClicked(boolean z, ColorLabel arrayLabelW[][], ColorLabel arrayLabelB[][], int a, int b, int x, JButton buttonW, JButton buttonB, JLabel subW, JLabel subB)
	{
		isWhite = z;
	...
	{	
		if(isWhite == true)
		{
			for(int i = x; i >= 1; i--)
			{
				arrayLabelW[a - i][b].setWhite();
				arrayLabelB[7 - (a - i)][7 - b].setWhite();
			}
		...
		else
		{
			for(int i = x; i >= 1; i--)
			{
				arrayLabelB[a - i][b].setBlack();
				arrayLabelW[7 - (a - i)][7 - b].setBlack();
			}
